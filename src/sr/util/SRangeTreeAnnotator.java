...
import beastfx.app.treeannotator.*;
 ...
 */
public class SRangeTreeAnnotator extends Console {
...
//    private final String locationOutputAttribute = "location";
    ...
    public SRangeTreeAnnotator(final int burninPercentage,
                    ...
                    if (tree.getDirectAncestorNodeCount() > 0 && !SAmode && processSA) {
                        ...
                        if (heightsOption == HeightsSummary.CA_HEIGHTS) {
                            throw new RuntimeException("""
                                    The common ancestor height is not\s
                                     available for trees with sampled ancestors. Please choose\s
                                     another height summary option""");
                        }
                        cladeSystem.setProcessSA(true);
                    }
                    cladeSystem.setTaxonNames(tree);
...
//                                    }
    ...
    static private sr.util.Controller controller;
        ...
        try {
            ...
            new Thread() {
                public void run() {
                    ...
                    try {
                        new SRangeTreeAnnotator(burninPercentage,
                                lowMem,
                                heightsOption,
                                posteriorLimit,
                                hpd2D,
    ...
    public static void main(String[] args) {
        ...
        Arguments arguments = new Arguments(
                new Arguments.Option[]{
                        ...
                        new Arguments.StringOption("heights", new String[]{"keep", "median", "mean", "ca"}, false,
                                "an option of 'ca' (default), 'median', 'mean' or 'keep'"),
        ...
        switch (args2.length) {
            ...
            case 1:
                ...
                break;
            default: {
                Log.err.println("\nCommand not understood: " + args2[2] + "\n");
                Log.err.println();
                printUsage(arguments);
                System.exit(1);
            }
        ...
        try {
            new SRangeTreeAnnotator(burnin, lowMem, heights, posteriorLimit, hpd2D, target, targetTreeFileName, inputFileName, outputFileName);
            //} catch (IOException e) {
            //	throw e;
        } catch (Exception e) {
            e.printStackTrace();
