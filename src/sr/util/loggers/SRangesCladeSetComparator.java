...
@Description("Match clades from two SR tree sets and print support for both sets so "
        + "they can be plotted in an X-Y plot")
public class SRangesCladeSetComparator extends Runnable {
    final public Input<TreeFile> src1Input = new Input<>("sRangesTree1","source sRanges tree 1 (set or MCC tree) file\"");
    final public Input<TreeFile> src2Input = new Input<>("sRangesTree2","source sRanges tree 2 (set or MCC tree) file");
    final public Input<String> src1Label = new Input<>("sRangesLabel1","Axis label for sRanges tree 1\"");
    final public Input<String> src2Label = new Input<>("sRangesLabel2", "Axis label for sRanges tree 2");
    ...
    final public Input<String> taxonSetSRanges1Input = new Input<>("taxonsetSR1",
            "comma separated taxa in sRanges tree 1 that are equivalent to taxons in SR tree 2" +
                    "Use only if there are spelling differences or species renaming.",
            "");

    final public Input<String> taxonSetSRanges2Input = new Input<>("taxonsetSR2",
            "comma separated taxa in sRanges tree 2 that are equivalent to taxons in SR tree 1 " +
                    "Use only if there are spelling differences or species renaming.",
            "");
...
//			"  <circle cx=\"677\" cy=\"104\" data-value=\"6.7\" r=\"4\"></circle>\n" +
    ...
    HashMap<String, String> SRangesTaxonMap = new HashMap<>();
    ...
    public void run() throws Exception {
        ...
        if ((!taxonSetSRanges1Input.get().isEmpty() && taxonSetSRanges2Input.get().isEmpty()) ||
                (taxonSetSRanges1Input.get().isEmpty() && !taxonSetSRanges2Input.get().isEmpty())) {
            throw new IllegalArgumentException("taxonSetSRanges1 and taxonSetSRanges2 must be both specified or both empty");
        }

        if (!taxonSetSRanges1Input.get().isEmpty() && !taxonSetSRanges2Input.get().isEmpty()){
            List<String> keys = new ArrayList<String>(Arrays.asList(taxonSetSRanges1Input.get().split(",")));
            List<String> values = new ArrayList<String>(Arrays.asList(taxonSetSRanges2Input.get().split(",")));
            if (keys.size() != values.size()) {
                throw new IllegalArgumentException("taxonSetSRanges1 and taxonSetSRanges2 must have the same number of elements");
            }
            for (int i = 0; i < keys.size(); i++) {
                SRangesTaxonMap.put(keys.get(i), values.get(i));
            }
        }
...
//		double n2 = n;
        ...
        for (int i = 0; i < cladeSet1.getCladeCount(); i++) {
            String clade = cladeSet1.getClade(i, SRangesTaxonMap);
        ...
        for (int i = 0; i < cladeSet2.getCladeCount(); i++) {
            String clade = cladeSet2.getClade(i, SRangesTaxonMap);
            double support2;
    ...
    private void initPNG(Graphics2D g, TreeFile tree1, TreeFile tree2) {
        ...
        g.drawString(src1Label.get(), 400, 1190);

        AffineTransform orig = g.getTransform();
        g.rotate(-Math.PI/2);
        g.drawString(src2Label.get(), -680, 40);
        g.setTransform(orig);
...
//			(support2 < 0.1 && support1 > 0.9)) {
    ...
    public class CladeSetWithHeights extends FrequencySet<BitSet> {
        ...
        /** get clade bit set */
        public String getClade(int index, HashMap<String,String> taxonMap) {
            BitSet bits = get(index);

            StringBuffer buffer = new StringBuffer("{");
            boolean first = true;
            for (String taxonId : getTaxaSet(bits, taxonMap)) {
                if (!first) {
                    buffer.append(", ");
                } else {
                    first = false;
        ...
        private SortedSet<String> getTaxaSet(BitSet bits, HashMap<String,String> taxonMap){
...
//	    }
    ...
    public static void main(String[] args) throws Exception {
        new Application(new SRangesCladeSetComparator(), "Clade Set Comparator", args);
